...
channel.queue('', exclusive: true).bind(exchange,
                                        routing_key: 'americas.north.#').subscribe do |delivery_info, _metadata, payload|
...
channel.queue('americas.south').bind(exchange,
                                     routing_key: 'americas.south.#').subscribe do |delivery_info, _metadata, payload|
  puts "An update for South America: #{payload}, routing key is #{delivery_info.routing_key}"
end
channel.queue('us.california').bind(exchange,
                                    routing_key: 'americas.north.us.ca.*').subscribe do |delivery_info, _metadata, payload|
  puts "An update for US/California: #{payload}, routing key is #{delivery_info.routing_key}"
end
channel.queue('us.tx.austin').bind(exchange,
                                   routing_key: '#.tx.austin').subscribe do |delivery_info, _metadata, payload|
  puts "An update for Austin, TX: #{payload}, routing key is #{delivery_info.routing_key}"
end
channel.queue('it.rome').bind(exchange,
                              routing_key: 'europe.italy.rome').subscribe do |delivery_info, _metadata, payload|
  puts "An update for Rome, Italy: #{payload}, routing key is #{delivery_info.routing_key}"
end
channel.queue('asia.hk').bind(exchange,
                              routing_key: 'asia.southeast.hk.#').subscribe do |delivery_info, _metadata, payload|
